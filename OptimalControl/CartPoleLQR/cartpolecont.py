...
class CartPoleContEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.theta_threshold_radians = 12 * 3 * math.pi / 360
        ...
        self.action_space = spaces.Box(shape=(1, 1), high=np.inf, low=-np.inf)
    ...
    def reset(self):
        ...
        self.state = np.array([0, 0, 0, -math.pi/3]).reshape(4,1)
        self.steps_beyond_done = None
        return self.state
    ...
    def set_state(self, state):
        self.state = np.concatenate((state[0], state[1], state[2], state[3])).reshape(4, 1)
