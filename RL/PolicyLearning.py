...
if __name__ == "__main__":
    ...
    name += f"_{subj}_custumshape"
    current_path = os.path.dirname(__file__)
    log_dir = os.path.join(current_path, env_type, "tmp", "log", name, algo_type)
    os.makedirs(log_dir, exist_ok=True)
    algo = SoftQiter(env, gamma=0.999, alpha=0.001, device=device)
    n = 1
    ...
    # for i in range(20):
    algo.learn(total_timesteps=int(50000), tb_log_name="extra", callback=save_policy_callback, reset_num_timesteps=False)
