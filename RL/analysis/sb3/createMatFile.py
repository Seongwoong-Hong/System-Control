...
if __name__ == "__main__":
    ...
    save_target = f"hipstiff30/{target}"
    name_tail = f"ppo_MinEffort_ptb1to7/{target}/const1.0_vel0.1_atm100"
    ...
    obs, _, _, _, ifs = exec_policy(env, agent, render=None, deterministic=True, repeat_num=len(load_kwargs['env_kwargs']['trials']), infos=['torque'])
    tqs = ifs['torque']

    bsps = []
    pltdds = [[] for _ in range(35)]
    pltqs = [[] for _ in range(35)]
    for trial in load_kwargs['env_kwargs']['trials']:
    ...
    tg_dir = Path(f"../MATLAB/{load_kwargs['env_id']}/{save_target}/sub10")
