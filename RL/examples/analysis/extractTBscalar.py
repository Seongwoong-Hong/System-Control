    ...
    for sw1 in [0.1]:
        for sw2 in [0.0]:
            ...
            for idx, file_path in enumerate(file_paths):
                ...
                ea.Reload()
                ev1s = ea.Scalars("episode_lengths/step")
                ev2s = ea.Scalars("rewards/step")
                episode_lengths = []
                rewards = []
                steps = []
                for e1, e2 in zip(ev1s, ev2s):
                    episode_lengths.append(e1.value)
                    steps.append(e1.step)
                    rewards.append(e2.value)

                data = {
                    'episode_length': np.array(episode_lengths),
