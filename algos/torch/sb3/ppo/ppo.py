...
from stable_baselines3.common.utils import logger
...
class PPOCustom(PPO):
    ...
    def _setup_learn(
        ...
        if original_start_time is None or reset_num_timesteps:
            original_start_time = time.time()
        total_timesteps, callback = \
            super()._setup_learn(
                total_timesteps,
                eval_env,
                callback,
    ...
    def reset_except_policy_param(self, env, **init_kwargs):
        self.init_kwargs['env'] = env
        if 'ent_schedule' in self.init_kwargs:
            self.ent_schedule = self.init_kwargs.pop('ent_schedule')
        else:
        ...
        for key, value in init_kwargs.items():
            if key in self.init_kwargs.keys():
                self.init_kwargs[key] = value
        super(PPOCustom, self).__init__(*self.init_args, **self.init_kwargs)
        self.policy.load_from_vector(param_vec)

    def reset_std(self, env, log_std: float = None):
        self.init_kwargs['env'] = env
        if 'ent_schedule' in self.init_kwargs:
            self.ent_schedule = self.init_kwargs.pop('ent_schedule')
        else:
        ...
        if log_std is None:
            ...
            state_dict['log_std'] = 0.2 * log_std_init + 0.8 * state_dict['log_std']
        else:
            state_dict['log_std'] = log_std
        super(PPOCustom, self).__init__(*self.init_args, **self.init_kwargs)
