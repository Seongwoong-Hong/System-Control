...
class Viter:
    ...
    def train(self):
        self.policy.q_table = self.reward_mat + self.gamma * (1 - self.done_mat) * \
                              np.sum(
                                  self.transition_mat * np.max(self.policy.q_table, axis=-1)[:, np.newaxis, np.newaxis],
                                  axis=0)
...
class SoftQiter(Viter):
    ...
    def train(self):
        self.policy.q_table = self.reward_mat + self.gamma * (1 - self.done_mat) * self.alpha * np.sum(
            self.transition_mat * np.log(np.sum(np.exp(self.policy.q_table / self.alpha), axis=-1))[:, None, None],
            axis=0)
        self.policy.v_table = np.log(np.sum(np.exp(self.policy.q_table), axis=-1))
        self.policy.policy_table = self.policy.softmax(self.policy.q_table)
...
class FiniteSoftQiter(Viter):
    ...
    def train(self):
        ...
        self.policy.v_table[self.T - 1, :] = self.logsumexp(self.policy.q_table[self.T - 1, :, :], axis=1)
        for t in reversed(range(self.T - 1)):
            ...
            self.policy.v_table[t, :] = self.logsumexp(self.policy.q_table[t, :, :], axis=1)
        self.policy.policy_table = np.exp(self.policy.q_table - self.policy.v_table[:, :, None])
