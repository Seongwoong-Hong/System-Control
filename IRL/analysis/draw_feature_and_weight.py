...
def draw_reward_weights():
    unnormalize = True
    log_dir = os.path.join(irl_path, "tmp", "log", "DiscretizedHuman", "MaxEntIRL", "sq_handnorm_a1_")
    ...
    for subj in label_name:
        ...
        for actuation in range(1, 2):
            ...
            for trial in [1, 2, 3, 4, 5]:
                ...
                if unnormalize:
                    gains = feature_fn(th.Tensor([[0.16, 0.7, 0.8, 2.4]]))
                    reward_weight /= gains
    ...
    x1 = [f"$\omega_{{{i + 1}}}$" for i in range(25)]
