...
def main(actuation=1):
    ...
    with open(f"{proj_path}/IRL/demos/DiscretizedHuman/19171717/{subj}_{actuation}.pkl", "rb") as f:
        expt_trajs = pickle.load(f)
    init_states = []
    for traj in expt_trajs:
        init_states += [traj.obs[0]]
    bsp = io.loadmat(subpath + f"i1.mat")['bsp']
    venv = make_env(env_name=f"{name}-v0", num_envs=1, N=[19, 17, 17, 17], bsp=bsp, init_states=init_states)
    ...
    with open(f"{proj_path}/RL/{env_type}/tmp/log/{name}_{subj}_19171717/softqiter/policies_1/agent.pkl", "rb") as f:
    ...
    save_name = f"{env_type}/19171717_quadcost/{subj}_{actuation}.pkl"
