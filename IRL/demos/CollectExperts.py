...
if __name__ == "__main__":
    ...
    env = make_env(env_name=f"{name}-v0", use_vec_env=False, subpath=subpath, wrapper=wrapper)
    sample_until = rollout.make_sample_until(n_timesteps=None, n_episodes=n_episodes)
    proj_path = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    with open(f"{proj_path}/../RL/{env_type}/tmp/log/{name}/softqlearning/policies_1/agent.pkl", "rb") as f:
    ...
    save_name = f"{env_type}/softqlearning_disc.pkl"
