...
if __name__ == '__main__':
    ...
    for subi in [5]:
        ...
        for actuation in range(4, 5):
            ...
            trajectories = []
            target_data = f"IDP/lqr_{subj}.pkl"
            with open(target_data, "rb") as f:
                trajs = pickle.load(f)
            for traj in trajs:
                obs = env.get_obs_from_idx(env.get_idx_from_obs(traj.obs))
                acts = env.get_acts_from_idx(env.get_idx_from_acts(traj.acts*100))
            ...
            save_name = f"{env_type}/19191919_lqr/quadcost_from_contlqr_sub05.pkl"
