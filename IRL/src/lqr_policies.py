...
class IDPiterLQRPolicy(iterLQRPolicy):
    def _build_env(self) -> np.array:
        ...
        self.A, self.B, _, _, dt = signal.cont2discrete((A, B, np.eye(4), 0), self.env.envs[0].dt)
        ...
        self.lf = lambda x: th.sum((x @ self.Q) * x, dim=-1)
        self.max_t = 720
        self.gear = 300
...
class IPLQRPolicy(DiscreteLQRPolicy):
    def _build_env(self):
        ...
        # self.Q = np.diag([0.7139, 1.0639979])
        # self.R = np.diag([.061537065])
        self.gear = 300
