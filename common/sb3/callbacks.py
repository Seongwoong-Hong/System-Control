...
class PolicyEvalCallback(BaseCallback):
    def __init__(self, trials, env_id, env_type, subj, use_norm, env_kwargs, *args, **kwargs):
        super().__init__()
        subpath = MAIN_DIR / "demos" / env_type / subj / subj
        if env_type == "IDPPD":
            subpath = MAIN_DIR / "demos" / env_type / subj / subj
        humanStates = [None for _ in range(35)]
        self.states = []
        self.torques = []
        for trial in trials:
            humanData = io.loadmat(str(subpath) + f"i{trial}.mat")
            bsp = humanData['bsp']
            humanStates[trial - 1] = humanData['state']
            self.states.append(humanData['state'])
            self.torques.append(humanData['tq'])
        self.use_norm = str2bool(use_norm)
        self.eval_env = make_env(f"{env_type}_{env_id}-v0", num_envs=1, bsp=bsp,
                                 humanStates=humanStates, use_norm=False, **env_kwargs)
        self.trials = trials
