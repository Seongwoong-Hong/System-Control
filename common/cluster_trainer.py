...
from ClusterStatistics import ClusterStatistics
from path_config import MAIN_DIR
from common.sb3.util import dot_map_dict_to_nested_dict, sample_config, list_of_dicts__to__dict_of_lists, yes_or_no, \
...
class ClusterTrainer:
    ...
    def run(self, train_py, name, num_samples, config, py_args='', overwrite=False):
        self.logger.info(f"train session of {name} started (#samples: {num_samples}, -1: grid-search)")

        # directory setup, handle overwriting
        exp_dir = self.main_dir / name / self.train_session
        if exp_dir.exists() and not overwrite:
            yes = yes_or_no(f'confirm to overwrite on {exp_dir}')
            if not yes:
                exp_dir = self.main_dir / name / self.creation_time
        ...
        if num_samples < 0:
            values = [[v] if not hasattr(v, '__iter__') or type(v) is str else v for v in config.values()]
            combinations = itertools.product(*values)
            config_list = [dict(zip(config.keys(), combo)) for combo in combinations]
            self.logger.info(f"all possible combinations is {len(config_list)} ways")
        else:
            config_list = [sample_config(config) for _ in range(num_samples)]

        dict_list = []
        for c in config_list:
            job_id = self.submit_job(train_py, c, py_args)
            ...
            c['job_dir'] = str(job_dir)
            varaint_json = job_dir.joinpath('variant.json')
            varaint_json.write_text(json.dumps(c, indent=True))

            c = nested_dict_to_dot_map_dict(c)
            dict_list.append(c)

        dict_list = list_of_dicts__to__dict_of_lists(dict_list)
        self.stat = ClusterStatistics(config=dict_list, logger_root='session.trainer')
